...
class Schema {
    ...
    uniqueReferences() {
        ...
        function addTablePrimaryKeys(table, index, array) {
            ...
            const r = new Reference(table.tableName, table.tableName, table.tableName, undefined, pk.originalFieldName, pk.originalFieldName, false, this);
            u.push(r);
        }
    }
}
...
exports.Table = Table;
class Field {
    constructor(originalFieldName, fieldName, fieldType, table, isPrimaryKey, referencedCol = false, isCalculated = false) {
...
exports.Field = Field;
class Reference {
    constructor(primaryTableName, foreignTableName, antiCollisionAssociationName, associationName, primaryKey, foreignKey, isView, schema) {
        this.primaryTableName = primaryTableName;
        this.foreignTableName = foreignTableName;
        this.antiCollisionAssociationName = antiCollisionAssociationName;
    ...
    antiCollisionAssociationNameCamel() {
...
function read(database, username, password, options, callback) {
    ...
    const idFieldLookup = {};
    const sql = "select table_name, column_name, data_type, ordinal_position, column_key " +
    ...
    function processTablesAndColumnsWithCustom(rows, customFieldLookup) {
        ...
        for (let index = 0; index < rows.length; index++) {
            ...
            const field = new Field(row.column_name, toCamelCase(row.column_name), row.data_type, table, !!row.column_key, false, isCalculated);
    ...
    function processReferences(rows) {
        ...
        function processReferenceRow(row) {
            ...
            schema.references.push(new Reference(row.referenced_table_name, row.table_name, antiCollisionAssociationName, toCamelCase(singularize(associationName === undefined
                ? row.referenced_table_name // Accounts -> account

...
class Schema {
    ...
    uniqueReferences() {
        ...
        function addTablePrimaryKeys(table, index, array) {
            ...
            const r = new Reference(table.tableName, table.tableName, undefined, pk.fieldName, pk.fieldName, false, this);
            u.push(r);
        }
    }
}
...
class Field {
    ...
    sequelizeFieldType() {
        let translated = `{type: ${Schema.fieldTypeSequelize[this.fieldType]}, field: '${this.originalFieldName}'}`;
        if (translated == undefined) {
...
function read(database, username, password, options, callback) {
        ...
        function processReferenceRow(row) {
            ...
            if (!row.hasOwnProperty('ordinal_position')) {
                ...
                if (!parentTable.fields.some(f => f.fieldName === fieldName)) {
                    parentTable.fields.push(new Field(fieldName, util.camelCase(row.table_name), // Leads -> leads
            ...
            schema.references.push(new Reference(row.referenced_table_name, row.table_name, associationName === undefined
                ? row.referenced_table_name // Accounts -> account
                : associationName, toCamelCase(Sequelize.Utils.singularize(row.referenced_table_name)) + toTitleCase(Schema.idSuffix), row.column_name, false, schema));
        }
    ...
    function addViewReference(view, index, array) {
        ...
        function addViewFieldReference(field, index, array) {
            ...
            const reference = new Reference(otherTableName, view.tableName, undefined, field.fieldName, field.fieldName, true, schema);
            schema.references.push(reference);
            const otherTableSingular = Sequelize.Utils.singularize(otherTableName);

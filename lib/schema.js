...
var Sequelize = sequelize_1["default"];
...
var Table = (function () {
    ...
    Table.prototype.tableNameUpperCase = function () {
    ...
    Table.prototype.realDbFields = function () {
        return this.fields.filter(function (f) { return !f.referencedCol && !f.isCalculated; });
    };
...
exports.Table = Table;
var Field = (function () {
    function Field(originalFieldName, fieldName, fieldType, table, referencedCol, isCalculated) {
        if (referencedCol === void 0) { referencedCol = false; }
    ...
    Field.prototype.sequelizeFieldType = function () {
        var translated = "{type: " + Schema.fieldTypeSequelize[this.fieldType] + ", field: \"" + this.originalFieldName + "\"}";
        if (translated == undefined) {
    ...
    Field.prototype.defineFieldType = function () {
        if (this == this.table.fields[0]) {
            var originalFieldName = this.table.fields[0].originalFieldName;
            ...
            var originalFieldName = this.table.fields[1].originalFieldName;
    ...
    Field.prototype.tableNameUpperCase = function () {
...
function read(database, username, password, options, callback) {
    ...
    sequelize
        ...
        .then(processTablesAndColumns)["catch"](function (err) {
        callback(err, null);
        return;
    });
    ...
    function readCustomFields(originalRows) {
        if (!_.some(originalRows, function (r) { return r.table_name == 'SequelizeCustomFieldDefinitions'; })) {
        ...
        sequelize
            .query(sql, { type: Sequelize.QueryTypes.SELECT })
            .then(processCustomFields)["catch"](function (err) {
            callback(err, null);
            return;
        });
        function processCustomFields(customFields) {
            ...
            customReferenceRows = _.filter(customFields, function (cf) { return cf.referenced_table_name != null && cf.referenced_column_name != null; });
    ...
    function processTablesAndColumnsWithCustom(rows, customFieldLookup) {
        ...
        for (var index = 0; index < rows.length; index++) {
            ...
            var field = new Field(row.column_name, toCamelCase(row.column_name), row.data_type, table, false, isCalculated);
    ...
    function readReferences() {
        ...
        sequelize
            .query(sql, { type: Sequelize.QueryTypes.SELECT })
            .then(processReferences)["catch"](function (err) {
            callback(err, null);
            return;
        });
    }
    function processReferences(rows) {
        ...
        function processReferenceRow(row) {
            ...
            if (!row.hasOwnProperty('ordinal_position')) {
                var lastField = parentTable.fields[parentTable.fields.length - 1];
                var fieldType = Sequelize.Utils.singularize(row.table_name) + 'Pojo[]';
                ...
                var field = new Field(row.column_name, fieldName, fieldType, // Leads -> LeadPojo[]
                parentTable, // Accounts table reference
                row.column_name);
                parentTable.fields.push(field);
            }
            // create singular parent reference from child
            childTable.fields.push(new Field(row.column_name, toCamelCase(Sequelize.Utils.singularize(associationName === undefined
            ...
            schema.references.push(new Reference(row.referenced_table_name, row.table_name, associationName, toCamelCase(Sequelize.Utils.singularize(row.referenced_table_name)) + toTitleCase(Schema.idSuffix), row.column_name, false, schema));
        }
        ...
        function processReferenceXrefs() {
            for (var xrefName in xrefs) {
                ...
                firstTable.fields.push(new Field(xref.secondTableName, toCamelCase(xref.secondTableName), Sequelize.Utils.singularize(xref.secondTableName) + 'Pojo[]', firstTable, true));
                secondTable.fields.push(new Field(xref.firstTableName, toCamelCase(xref.firstTableName), Sequelize.Utils.singularize(xref.firstTableName) + 'Pojo[]', secondTable, true));
            }
        }
    }
    ...
    function addViewReference(view, index, array) {
        ...
        function addViewFieldReference(field, index, array) {
            ...
            view.fields.push(new Field(otherTableSingular, toCamelCase(otherTableSingular), otherTableSingular + 'Pojo', view, true));
            otherTable.fields.push(new Field(view.tableName, toCamelCase(view.tableName), Sequelize.Utils.singularize(view.tableName) + 'Pojo[]', otherTable, true));
        }
    }
...
function toCamelCase(text) {
    var lowerCaseStr = text.charAt(0).toLowerCase() + text.substr(1);
    return lowerCaseStr.replace(/(\_\w)/g, function (m) { return m[1].toUpperCase(); });
}
